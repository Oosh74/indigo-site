...
async function moveBlogLayouts() {
  try {
    ...
    const blogLayoutFiles = files.filter(
      (file) => file.startsWith("Blog") && file.endsWith(".astro"),
    );
...
async function scanForReferences(removedBlogContent) {
  ...
  for (const file of files) {
    try {
      ...
      if (
        removedBlogContent &&
        content.match(/from\s+["'].*\/layouts\/Blog.*["']/)
      ) {
    ...
    if (hasOtherCollections) {
      ...
      if (collectionMatch) {
        ...
        const collectionEntries =
          collectionsContent.match(/\w+\s*:\s*\w+/g) || [];
        ...
        if (collectionEntries.length === 1 && content.includes("blog:")) {
          ...
          console.log(
            "Deleted content.config.ts (no other collections remaining)",
          );
          ...
          updatedContent = updatedContent.replace(
            /\s*blog\s*:\s*blogsCollection\s*,?\s*/g,
            "",
          );
          ...
          console.log(
            "Updated content.config.ts (removed blog collection, kept other collections)",
          );
        }
      }
    } else {
...
async function cleanupNavData() {
  ...
  try {
    ...
    await fs.writeFile(
      navDataPath,
      JSON.stringify(filteredNavData, null, 2) + "\n",
      "utf-8",
    );
...
async function removeDecapCMS() {
  ...
  const removeBlogContent = await new Promise((resolve) => {
    rl.question(
      "Do you also want to remove all blog-related content and config files? (choose no if you want to run local Content Collections without Decap) (y/n): ",
      (answer) => {
        rl.close();
        resolve(answer.toLowerCase() === "y");
      },
  ...
  try {
    ...
    try {
      ...
      if (error.code === "ENOENT") {
        console.log(
          `Admin folder not found at ${adminSourcePath}, skipping...`,
        );
      } else {
    ...
    if (removeBlogContent) {
      ...
      try {
        ...
        if (error.code === "ENOENT") {
          console.log(
            `Blog content folder not found at ${blogContentPath}, skipping...`,
          );
        } else {
      ...
      try {
        ...
        if (error.code === "ENOENT") {
          console.log(
            `Blog pages folder not found at ${blogPagesPath}, skipping...`,
          );
        } else {
  ...
  try {
    ...
    const { decapReferences, blogReferences } =
      await scanForReferences(removeBlogContent);
    ...
    // Next steps
    if (
      decapReferences.length > 0 ||
      (blogReferences.length > 0 && removeBlogContent)
    ) {
      ...
      if (blogReferences.length > 0 && removeBlogContent) {
        console.log(
          "   - Fix files with remaining blog imports (auto-cleanup attempted)",
        );
      }
      console.log();
    }
